...
stru_t update_env_element(stru_t stru, int pwd_line, char *new_dir)
{
    int x = 0;
    char pwd[] = "PWD";
...
stru_t cd_bultin_two(stru_t stru)
{
    if (nb_tab_lines(stru.line) == 2 && stru.line[1][0] != '~') {
        ...
        if (chdir(stru.line[1]) == -1) {
            ...
            stru = update_env_element(stru, search_pwd_element(stru), stru.pwd);
        }
    }
    return (stru);
}
